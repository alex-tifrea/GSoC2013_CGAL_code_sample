...
FT klein_function (Point_3 p) {
  return (p.x()*p.x()+p.y()*p.y()+p.z()*p.z()+2*p.y()-1)*((p.x()*p.x()+p.y()*p.y()+p.z()*p.z()-2*p.y()-1)*(p.x()*p.x()+p.y()*p.y()+p.z()*p.z()-2*p.y()-1)-8*p.z()*p.z())+16*p.x()*p.z()*(p.x()*p.x()+p.y()*p.y()+p.z()*p.z()-2*p.y()-1);
}
